...
class TestBenchmark(unittest.TestCase):
    ...
    def test_add_train_accuracy(self):
        ...
        self.mock_state.set.assert_called_with("acc_train", [0.95, 0.96])
    ...
    def test_add_train_loss(self):
        ...
        self.mock_state.set.assert_called_with("train_loss", [0.1, 0.08])
    ...
    def test_add_test_accuracy(self):
        ...
        self.mock_state.set.assert_called_with("acc_test", [0.9, 0.92])
    ...
    def test_gpu_memory_usage(self, mock_reset_peak_memory, mock_max_memory_allocated, mock_device_count):
        ...
        self.assertAlmostEqual(self.benchmark.averageMemory.get(), 10.0, delta=0.1) 

    def test_flush(self):
        self.benchmark.averageStepTime = MeanAggregator(measure=lambda x: x)
        self.benchmark.averageStepTime(0.02)  
        self.benchmark.averageMemory = MeanAggregator(measure=lambda x: x)
        self.benchmark.averageMemory(10.0)  

        self.benchmark.flush()

        self.mock_state.set.assert_any_call("tps", [0.02])
        self.mock_state.set.assert_any_call("gpu_mem", [10.0])
